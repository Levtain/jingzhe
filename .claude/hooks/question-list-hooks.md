# 问题清单自动化Hooks配置

> **版本**: v1.0
> **创建时间**: 2025-01-11
> **目的**: 自动化问题清单核实,避免重复讨论

---

## 🎯 设计目标

在以下关键时机自动触发问题清单核实,确保不会重复讨论已确认的问题:

1. ✅ **创建问题清单后** - 立即核实是否有已确认的问题
2. ✅ **开始讨论前** - 确保问题清单状态最新
3. ✅ **完成讨论后** - 最终核实并同步所有确认

---

## 🔧 Hook配置

### Hook 1: 问题清单创建后自动核实

**触发条件**:
```yaml
event: FileCreated
file_pattern: development/issues/*questions*.md
```

**自动执行**:
```bash
question-verification-agent $FILE_PATH
```

**流程**:
```
用户创建问题清单
  ↓
文件创建事件触发
  ↓
question-verification-agent 自动启动
  ↓
扫描所有相关文档
  ↓
生成核实报告
  ↓
询问用户是否更新问题清单
  ↓
[用户确认] → 自动更新问题清单状态
```

**示例输出**:
```markdown
⚡ 检测到新问题清单: game-submission-questions-v2.md

🔍 自动启动问题清单核实...

扫描文档: 15个
发现问题: 9个
✅ 已确认: 7个
⏳ 待确认: 2个

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

【核实结果】

已在其他文档确认的问题:
- Q8: 硬核玩家标记的作用和奖励机制?
  → 确认来源: a-level-problems-resolution-summary_2025-01-10.md
  → 确认时间: 2025-01-10

- Q9: 源码链接的有效性验证?
  → 确认来源: questions.md
  → 确认时间: 2025-01-09

仍需讨论的问题:
- Q1: 游戏提交时的作者列表如何生成?
- Q2: 赛道选择是手动选择还是自动判定?

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

❓ 是否自动更新问题清单状态?
- 输入 "是" / "yes" / "y" → 自动更新
- 输入 "否" / "no" / "n" → 跳过更新
```

---

### Hook 2: discussion-agent 启动前预核实

**触发条件**:
```yaml
event: AgentInvoked
agent: discussion-agent
```

**自动执行**:
```python
# discussion-agent 内部逻辑
def start_discussion():
    # 步骤1: 先运行核实
    verification = question_verification_agent.verify()

    # 步骤2: 展示核实结果
    if verification.has_confirmed_questions():
        show_verification_results(verification)

        # 步骤3: 询问是否更新
        if user_confirms_update():
            update_question_list(verification)

    # 步骤4: 然后开始讨论
    start_with_unconfirmed_questions()
```

**流程**:
```
用户: "/discuss"
  ↓
discussion-agent 启动
  ↓
[内部] 调用 question-verification-agent
  ↓
返回核实结果
  ↓
展示已确认问题列表
  ↓
用户确认或跳过
  ↓
仅加载未确认的问题
  ↓
开始讨论
```

**集成代码**:
```python
# discussion-agent.md 核心逻辑更新

def load_next_unanswered_question():
    """
    加载下一个未确认的问题(已核实版本)
    """
    # 步骤1: 先运行核实
    verification = run_question_verification()

    # 步骤2: 过滤已确认的问题
    unconfirmed_questions = [
        q for q in questions
        if not verification.is_confirmed(q)
    ]

    # 步骤3: 展示核实摘要
    if verification.confirmed_count > 0:
        show_verification_summary(verification)

    # 步骤4: 返回第一个真正未确认的问题
    return unconfirmed_questions[0] if unconfirmed_questions else None
```

---

### Hook 3: 问题清单完成后最终核实

**触发条件**:
```yaml
event: QuestionListCompleted
condition: 所有问题标记 ✅
```

**自动执行**:
```bash
question-verification-agent --final-check $FILE_PATH
```

**流程**:
```
用户: "所有问题已确认"
  ↓
检测到100%完成
  ↓
question-verification-agent 最终核实
  ↓
交叉验证所有确认
  ↓
生成完成报告
  ↓
建议下一步操作
```

**输出**:
```markdown
🎉 问题清单核实完成

**问题清单**: game-submission-questions-v2.md
**完成度**: 100% (9/9)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

✅ 核实结果
- 所有问题已确认
- 所有确认已同步
- 无冲突发现
- 版本号一致

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

【建议下一步】

1. 运行 /sync-docs 同步所有文档
2. 运行 completion-check-agent 验证模块完整性
3. 开始创建设计文档

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

---

## 🛠️ 实施方案

### 阶段1: Agent集成 (已完成 ✅)

**已创建**:
- [x] `question-verification-agent.md` - 核实Agent
- [x] 更新 `discussion-agent.md` - 集成核实步骤

**待完成**:
- [ ] 更新 `discussion-agent.md` 的核心逻辑,添加自动调用
- [ ] 测试Agent之间的协调

### 阶段2: Hook实现 (进行中)

**技术方案**:

**方案A: discussion-agent内部集成** (推荐)
```python
# discussion-agent 的核心逻辑中添加
def start():
    # 1. 自动运行核实
    verification = verify_question_list()

    # 2. 展示结果
    if verification.has_confirmations():
        print(f"⚠️ 发现{verification.confirmed_count}个问题已确认")
        print("是否更新问题清单? (y/n)")

        if user_confirms():
            verification.apply_updates()

    # 3. 继续正常流程
    proceed_with_discussion()
```

**优点**:
- 简单直接,无需外部Hook系统
- Agent内部自动处理
- 用户体验流畅

**方案B: 文件系统监控** (备选)
```python
# 使用文件监控实现自动触发
from watchdog import observers, events

class QuestionListHandler(events.FileSystemEventHandler):
    def on_created(self, event):
        if event.src_path.endswith('questions.md'):
            # 新建问题清单,自动核实
            launch_verification_agent(event.src_path)
```

**优点**:
- 真正的事件驱动
- 完全自动化

**缺点**:
- 需要额外的监控进程
- 增加系统复杂度

### 阶段3: 用户命令 (快捷方式)

```bash
# 手动触发核实
/verify-questions [文件路径]

# 核实并自动更新
/verify-questions --update

# 仅显示报告,不修改文件
/verify-questions --report-only

# 核实最新问题清单
/verify-questions
```

---

## 📊 Hook优先级

```yaml
优先级1 (最高):
  - discussion-agent 内部自动核实
  - 每次讨论前必执行

优先级2 (推荐):
  - 问题清单创建后手动核实
  - 用户主动触发

优先级3 (可选):
  - 文件系统监控
  - 完全自动化(未来)
```

---

## 🧪 测试方案

### 测试用例1: 创建问题清单

**步骤**:
1. 创建新问题清单 `test-questions.md`
2. 预期: 自动触发核实Agent
3. 检查: 生成核实报告

**预期结果**:
```
⚡ 检测到新问题清单
🔍 自动启动问题清单核实...
[核实报告]
```

### 测试用例2: 讨论已确认问题

**步骤**:
1. 在 `game-submission-questions-v2.md` 中,Q8已确认
2. 用户调用: "/discuss"
3. 预期: discussion-agent 自动跳过Q8

**预期结果**:
```
加载问题清单...
⚠️ Q8已在其他文档确认
跳过Q8,加载Q9...
```

### 测试用例3: 完成讨论

**步骤**:
1. 所有问题标记为 ✅
2. 用户: "所有问题已确认"
3. 预期: 最终核实并生成完成报告

**预期结果**:
```
🎉 问题清单核实完成
100%完成度
建议下一步操作
```

---

## 🎯 实施优先级

### P0 (立即实施)
- [x] 创建 question-verification-agent
- [x] 更新 discussion-agent 添加核实步骤
- [ ] 测试基本核实功能

### P1 (短期实施)
- [ ] 添加手动触发命令 `/verify-questions`
- [ ] 完善核实报告格式
- [ ] 添加批量更新功能

### P2 (长期优化)
- [ ] 文件系统监控Hook
- [ ] 智能问题匹配算法
- [ ] 问题数据库统一管理

---

## 📝 配置文件示例

**`.claude/hooks/config.yaml`**:
```yaml
# 问题清单自动化Hooks配置

hooks:
  - name: question-list-verification
    trigger:
      event: AgentInvoked
      agent: discussion-agent
    action:
      type: verify_questions
      priority: high
    enabled: true

  - name: question-list-created
    trigger:
      event: FileCreated
      pattern: "development/issues/*questions*.md"
    action:
      type: verify_questions
      auto_update: false  # 询问用户后再更新
    enabled: false  # 暂时禁用,测试通过后启用

  - name: question-list-completed
    trigger:
      event: QuestionListCompleted
    action:
      type: verify_questions
      final_check: true
    enabled: true
```

---

## 🔄 工作流程对比

### 改进前

```yaml
创建问题清单:
  1. 创建文档
  2. 手动检查是否有重复
  3. 可能遗漏已确认问题

讨论问题:
  1. 调用 /discuss
  2. discussion-agent 加载问题
  3. 可能重复讨论已确认问题
  4. 用户提醒"这个问题确认过了"
  5. 手动搜索相关文档
  6. 浪费5-10分钟
```

### 改进后

```yaml
创建问题清单:
  1. 创建文档
  2. [自动] question-verification-agent 核实
  3. 自动发现已确认问题
  4. 用户确认后自动更新

讨论问题:
  1. 调用 /discuss
  2. [自动] discussion-agent 先核实
  3. 展示已确认问题列表
  4. 仅加载未确认问题
  5. 节省5-10分钟,用户满意度提升
```

---

## 📈 预期效果

### 效率提升
```yaml
问题核实时间:
  改进前: 5-10分钟手动搜索
  改进后: 10秒自动完成
  提升: 30-60倍

重复讨论次数:
  改进前: 每个问题清单2-3次重复
  改进后: 0次重复
  提升: 100%避免

用户满意度:
  改进前: "这个问题又讨论过了?" ❌
  改进后: "自动发现已确认问题" ✅
```

### 质量改进
```yaml
文档同步:
  改进前: 经常不同步
  改进后: 自动同步,一致性高

问题追踪:
  改进前: 分散在各文档
  改进后: 统一核实,清晰追踪

决策质量:
  改进前: 可能遗漏历史决策
  改进后: 完整展示所有相关决策
```

---

## 🔗 相关文档

- [question-verification-agent.md](../agents/question-verification-agent.md) - 核实Agent
- [discussion-agent.md](../agents/discussion-agent.md) - 讨论Agent(已集成)
- [question-list-verification-workflow_2025-01-11.md](../../development/workflow-improvements/question-list-verification-workflow_2025-01-11.md) - 工作流程改进

---

**文档创建时间**: 2025-01-11
**版本**: v1.0
**状态**: ✅ Agent已创建,集成方案已设计
**下一步**: 实施discussion-agent内部集成,测试验证
