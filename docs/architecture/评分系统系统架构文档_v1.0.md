# 惊蛰计划 - 评分系统系统架构文档

**文档版本**: v1.0
**创建日期**: 2025-01-10
**最后更新**: 2025-01-10
**文档状态**: 🔄 初稿
**相关文档**:
- [评分系统设计方案 v2.5](../design/评分系统设计方案_v1.0.md)
- [排名系统技术实现文档 v1.0](../design/archive/ranking_system_v1_deprecated/排名系统技术实现文档_v1.0.md)

---

## 文档说明

本文档定义惊蛰计划评分系统的系统架构,包括数据流水线、计算流程、缓存策略、监控指标、回滚流程等工程实现细节。

**适用范围**: 第一届评分系统(截尾均值 + 贝叶斯平滑 + 单维度排名)

---

## 一、系统架构总览

### 1.1 核心组件

```
┌─────────────────────────────────────────────────────────────┐
│                        前端层                                 │
│  - 评分页面(滑块交互)                                          │
│  - 榜单展示页面(Tab切换)                                       │
│  - 游戏详情页(评分统计)                                        │
└────────────────────┬────────────────────────────────────────┘
                     │ HTTP/REST API
┌────────────────────▼────────────────────────────────────────┐
│                      API网关层                                │
│  - 速率限制(10分钟内≤6个评分)                                  │
│  - 权限验证(参赛者/评审团)                                     │
│  - 阶段状态检查(评分阶段是否开放)                              │
└────────────────────┬────────────────────────────────────────┘
                     │
┌────────────────────▼────────────────────────────────────────┐
│                    业务逻辑层                                 │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐       │
│  │ 评分服务     │  │ 排名服务     │  │ 反作弊服务   │       │
│  │ - 提交评分   │  │ - 计算排名   │  │ - 监测异常   │       │
│  │ - 修改评分   │  │ - 缓存管理   │  │ - 标记可疑   │       │
│  │ - 查询评分   │  │ - 重算触发   │  │ - 复核流程   │       │
│  └──────────────┘  └──────────────┘  └──────────────┘       │
└────────────────────┬────────────────────────────────────────┘
                     │
┌────────────────────▼────────────────────────────────────────┐
│                    数据访问层                                 │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐       │
│  │ PostgreSQL   │  │    Redis     │  │  消息队列    │       │
│  │ - 评分数据   │  │ - 缓存排名   │  │ - 异步重算   │       │
│  │ - 游戏数据   │  │ - TTL 5分钟  │  │ - 通知发送   │       │
│  │ - 用户数据   │  │              │  │              │       │
│  └──────────────┘  └──────────────┘  └──────────────┘       │
└─────────────────────────────────────────────────────────────┘
```

### 1.2 数据流总览

```
评分提交
   ↓
[API网关] 权限验证 + 速率限制
   ↓
[评分服务] 数据验证 + 存储到PostgreSQL
   ↓
[消息队列] 触发异步重算任务
   ↓
[排名服务] 计算排名 → Redis缓存
   ↓
[前端轮询/WebSocket] 获取最新排名
```

---

## 二、数据流水线设计

### 2.1 数据模型

#### 核心表结构

```sql
-- 原始评分表
CREATE TABLE ratings (
    id BIGSERIAL PRIMARY KEY,
    game_id BIGINT NOT NULL,                    -- 游戏ID
    rater_id BIGINT NOT NULL,                   -- 评分者ID
    rater_type VARCHAR(20) NOT NULL,            -- 评分者类型: 'participant' | 'judge'

    -- 6个维度评分
    innovation_score DECIMAL(4,2),              -- 创新性
    theme_score DECIMAL(4,2),                   -- 主题诠释
    visual_score DECIMAL(4,2),                  -- 视觉效果
    audio_score DECIMAL(4,2),                   -- 音乐音频
    completeness_score DECIMAL(4,2),            -- 整体性
    fun_factor_score DECIMAL(4,2),              -- 整活儿

    -- 状态标记
    is_valid BOOLEAN DEFAULT true,              -- 是否有效
    is_suspicious BOOLEAN DEFAULT false,        -- 是否可疑
    suspicious_reason TEXT,                     -- 可疑原因

    -- 审计字段
    ip_address INET,
    user_agent TEXT,
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW(),

    -- 唯一约束: 每个评分者对每个游戏只能有一条有效评分
    UNIQUE (game_id, rater_id)
);

-- 游戏维度得分表(计算结果)
CREATE TABLE game_dimension_scores (
    id BIGSERIAL PRIMARY KEY,
    game_id BIGINT NOT NULL,
    dimension VARCHAR(20) NOT NULL,             -- 维度名称: 'innovation' | 'theme' | ...

    -- 原始统计
    raw_mean DECIMAL(5,3),                      -- 原始平均分
    raw_count INTEGER,                          -- 原始评分数量

    -- 截尾均值结果
    trimmed_mean DECIMAL(5,3),                  -- 截尾均值
    trimmed_count INTEGER,                      -- 截尾后数量
    variance DECIMAL(8,6),                      -- 方差

    -- 贝叶斯平滑结果
    bayesian_score DECIMAL(5,3),               -- 贝叶斯平滑分数
    global_mean DECIMAL(5,3),                   -- 全局均值

    -- 评审团统计
    judge_count INTEGER,                        -- 评审团人数
    judge_avg_score DECIMAL(5,3),               -- 评审团平均分

    -- 排名信息
    participant_rank INTEGER,                   -- 参赛者榜排名
    judge_rank INTEGER,                         -- 评审团榜排名

    -- 门槛判定
    total_rating_count INTEGER,                 -- 总评分人次(评审团×2)
    is_ranked BOOLEAN,                          -- 是否进入排名

    -- 缓存控制
    last_calculated_at TIMESTAMP,
    created_at TIMESTAMP DEFAULT NOW(),

    UNIQUE (game_id, dimension)
);

-- 排名榜单表(12个榜单)
CREATE TABLE rankings (
    id BIGSERIAL PRIMARY KEY,
    dimension VARCHAR(20) NOT NULL,             -- 维度
    group_type VARCHAR(20) NOT NULL,            -- 群体: 'participant' | 'judge'
    season_id VARCHAR(50) NOT NULL,             -- 赛季ID

    -- 排名数据(JSON存储,便于批量查询)
    ranking_data JSONB NOT NULL,                -- 格式: [{rank, game_id, score, ...}]

    -- 元数据
    total_games INTEGER,                        -- 总游戏数
    ranked_games INTEGER,                       -- 进入排名的游戏数
    last_game_score DECIMAL(5,3),              -- 最后一名得分

    -- 版本控制
    version INTEGER,                            -- 榜单版本(每次重算+1)
    created_at TIMESTAMP DEFAULT NOW(),

    UNIQUE (dimension, group_type, season_id)
);

-- 反作弊审计表
CREATE TABLE anti_cheat_logs (
    id BIGSERIAL PRIMARY KEY,
    rating_id BIGINT REFERENCES ratings(id),

    -- 检测类型
    detection_type VARCHAR(50),                 -- 'high_frequency' | 'low_scores' | 'manual_report'
    detection_details JSONB,                    -- 检测详情

    -- 处理状态
    status VARCHAR(20),                         -- 'pending' | 'confirmed' | 'rejected'
    handled_by BIGINT,                          -- 处理人ID
    handled_at TIMESTAMP,

    -- 审计
    created_at TIMESTAMP DEFAULT NOW()
);
```

#### 索引设计

```sql
-- 评分表索引
CREATE INDEX idx_ratings_game_id ON ratings(game_id);
CREATE INDEX idx_ratings_rater_id ON ratings(rater_id);
CREATE INDEX idx_ratings_is_valid ON ratings(is_valid) WHERE is_valid = false;
CREATE INDEX idx_ratings_is_suspicious ON ratings(is_suspicious) WHERE is_suspicious = true;
CREATE INDEX idx_ratings_created_at ON ratings(created_at DESC);

-- 游戏得分表索引
CREATE INDEX idx_game_scores_game_id ON game_dimension_scores(game_id);
CREATE INDEX idx_game_scores_is_ranked ON game_dimension_scores(is_ranked) WHERE is_ranked = true;
CREATE INDEX idx_game_scores_participant_rank ON game_dimension_scores(dimension, participant_rank);
CREATE INDEX idx_game_scores_judge_rank ON game_dimension_scores(dimension, judge_rank);

-- 榜单表索引
CREATE INDEX idx_rankings_dimension_group ON rankings(dimension, group_type, season_id);
CREATE INDEX idx_rankings_created_at ON rankings(created_at DESC);

-- 反作弊表索引
CREATE INDEX idx_anti_cheat_status ON anti_cheat_logs(status) WHERE status = 'pending';
CREATE INDEX idx_anti_cheat_created_at ON anti_cheat_logs(created_at DESC);
```

### 2.2 ETL流程

#### 流程图

```
┌─────────────────────────────────────────────────────────────┐
│ 1. 原始数据采集(ratings_raw)                                 │
│    - 评分数值                                                │
│    - 评分者信息                                              │
│    - 时间戳、IP等元数据                                      │
└────────────────────┬────────────────────────────────────────┘
                     ↓
┌─────────────────────────────────────────────────────────────┐
│ 2. 数据清洗与验证                                            │
│    - 排除自评(game.author_id = rater_id)                    │
│    - 检查评分范围(0-10, step 0.5)                           │
│    - 标记可疑评分(is_suspicious标记)                         │
│    - 过滤is_valid=false的评分                                │
└────────────────────┬────────────────────────────────────────┘
                     ↓
┌─────────────────────────────────────────────────────────────┐
│ 3. 截尾均值计算(Winsorized Mean)                             │
│    - 按game_id + dimension分组                              │
│    - 排序并移除上下各10%                                     │
│    - 计算剩余评分的算术平均                                  │
│    - 小样本(<10个)不截断                                     │
└────────────────────┬────────────────────────────────────────┘
                     ↓
┌─────────────────────────────────────────────────────────────┐
│ 4. 全局均值计算                                              │
│    - 仅包含通过门槛的游戏(>15人次)                           │
│    - 按dimension分组计算                                     │
│    - global_mean = Σ(trimmed_mean) / count                   │
└────────────────────┬────────────────────────────────────────┘
                     ↓
┌─────────────────────────────────────────────────────────────┐
│ 5. 贝叶斯平滑计算                                            │
│    - 公式: S' = (n×S + 20×μ) / (n+20)                        │
│    - n: 评分人次(评审团×2)                                   │
│    - S: 截尾均值                                             │
│    - μ: 全局均值                                             │
└────────────────────┬────────────────────────────────────────┘
                     ↓
┌─────────────────────────────────────────────────────────────┐
│ 6. 排名计算                                                  │
│    - 按dimension + group_type分组                           │
│    - 按bayesian_score降序排序                                │
│    - 处理并列(三级tie-breaker)                               │
│    - 判定优选游戏(3+个维度前10)                              │
└────────────────────┬────────────────────────────────────────┘
                     ↓
┌─────────────────────────────────────────────────────────────┐
│ 7. 榜单生成(12个榜单)                                        │
│    - 参赛者榜(6个维度)                                       │
│    - 评审团榜(6个维度)                                       │
│    - 优选游戏榜(2个群体)                                     │
└─────────────────────────────────────────────────────────────┘
```

#### 计算伪代码

```python
def recalculate_rankings(season_id: str):
    """
    全量重算排名
    """
    # 步骤1: 获取所有游戏
    games = get_all_games(season_id)

    # 步骤2: 对每个维度分别计算
    for dimension in ['innovation', 'theme', 'visual', 'audio', 'completeness', 'fun_factor']:
        # 2.1 计算全局均值(仅包含通过门槛的游戏)
        global_mean = calculate_global_mean(games, dimension, threshold=15)

        # 2.2 计算每个游戏的得分
        game_scores = []
        for game in games:
            # 2.2.1 获取有效评分(排除自评、可疑评分)
            valid_ratings = get_valid_ratings(game.id, dimension)

            # 2.2.2 计算评分人次(评审团×2)
            total_count = calculate_total_count(valid_ratings)

            # 2.2.3 检查排名资格
            if total_count <= 15:
                # 未达门槛,不参与排名
                save_unranked_game(game.id, dimension, valid_ratings)
                continue

            # 2.2.4 计算截尾均值
            trimmed_mean, variance = calculate_trimmed_mean(valid_ratings)

            # 2.2.5 贝叶斯平滑
            bayesian_score = apply_bayesian_smoothing(
                trimmed_mean, total_count, global_mean, k=20
            )

            # 2.2.6 保存得分
            game_scores.append({
                'game_id': game.id,
                'score': round(bayesian_score, 3),  # 保留3位小数
                'count': total_count,
                'variance': variance,
                'judge_ratio': calculate_judge_ratio(valid_ratings)
            })

        # 步骤3: 排序并处理并列
        ranked_games = sort_and_break_ties(game_scores)

        # 步骤4: 保存榜单
        save_ranking(dimension, 'participant', ranked_games, season_id)
        save_ranking(dimension, 'judge', ranked_games, season_id)

    # 步骤5: 计算优选游戏
    calculate_featured_games(games, season_id)
```

### 2.3 数据一致性保障

#### 原子性保证

```python
@transaction.atomic
def submit_rating(rating_data: RatingSubmitRequest) -> Rating:
    """
    提交评分(原子操作)
    """
    # 1. 验证权限
    validate_rating_permission(rating_data.rater_id, rating_data.game_id)

    # 2. 检查评分阶段状态
    check_scoring_phase_active()

    # 3. 检查速率限制(10分钟内≤6个评分)
    check_rate_limit(rating_data.rater_id)

    # 4. 保存评分
    rating = Rating.objects.create(**rating_data.dict())

    # 5. 触发异步重算任务
    trigger_recalculation_task(rating.game_id)

    return rating
```

#### 幂等性保证

```python
def trigger_recalculation_task(game_id: int):
    """
    触发重算任务(幂等)
    """
    # 使用Redis锁,防止重复触发
    lock_key = f"recalc_lock:{game_id}"
    lock = redis_client.lock(lock_key, timeout=300, blocking_timeout=1)

    if lock.acquire():
        try:
            # 发送消息到队列
            message_queue.publish({
                'action': 'recalculate',
                'game_id': game_id,
                'timestamp': datetime.now().isoformat()
            })
        finally:
            lock.release()
```

#### 数据校验

```python
def validate_data_consistency():
    """
    数据一致性校验(定时任务)
    """
    issues = []

    # 校验1: 评分总数 vs 人次总数
    for dimension in DIMENSIONS:
        ratings_count = Rating.objects.filter(
            dimension=dimension,
            is_valid=True
        ).count()

        total_count = GameDimensionScore.objects.aggregate(
            total=Sum('total_rating_count')
        )['total'] or 0

        if ratings_count != total_count:
            issues.append(f"维度{dimension}: 评分总数({ratings_count}) != 人次总数({total_count})")

    # 校验2: 榜单数据 vs 游戏得分数据
    for dimension in DIMENSIONS:
        for group_type in ['participant', 'judge']:
            ranking = get_latest_ranking(dimension, group_type)
            if not ranking:
                continue

            ranking_games = len(ranking.ranking_data)
            db_games = GameDimensionScore.objects.filter(
                dimension=dimension,
                is_ranked=True
            ).count()

            if ranking_games != db_games:
                issues.append(f"{dimension}-{group_type}: 榜单游戏数({ranking_games}) != DB游戏数({db_games})")

    # 如果发现不一致,发送告警
    if issues:
        send_alert("数据一致性校验失败", issues)
```

---

## 三、排名计算策略

### 3.1 计算频率

#### 评分阶段(排名前)

```python
# 异步触发机制
def on_rating_submitted(rating_id: int):
    """
    新评分提交后触发
    """
    game_id = get_game_id_by_rating(rating_id)

    # 异步任务: 仅重算受影响的游戏和维度
    async_recalculate_game.delay(game_id)

    # 清除缓存
    invalidate_ranking_cache(game_id)
```

**特点**:
- ✅ 实时响应(5分钟延迟)
- ✅ 性能优化(仅重算相关游戏)
- ✅ 最终一致性

#### 排名公布后

```python
# 定时任务
@app.task
def daily_recalculation():
    """
    每天凌晨2点全量重算
    """
    schedule.every().day.at("02:00").do(job)

    recalculate_all_rankings()
```

**特点**:
- ✅ 数据稳定性(每日全量)
- ✅ 防止数据漂移
- ✅ 支持数据修正

### 3.2 缓存策略

#### Redis缓存设计

```python
# 缓存键设计
CACHE_KEYS = {
    # 游戏得分(单个游戏)
    'game_score': 'game:scores:{game_id}',                    # TTL: 5分钟

    # 榜单(12个榜单)
    'ranking': 'ranking:{dimension}:{group_type}:{season_id}', # TTL: 5分钟

    # 全局均值(6个维度)
    'global_mean': 'global_mean:{dimension}:{season_id}',     # TTL: 1小时

    # 用户评分状态(是否已评分)
    'user_rated': 'user:rated:{user_id}:{game_id}',           # TTL: 1天
}

# 缓存更新策略
def update_ranking_cache(dimension: str, group_type: str, season_id: str):
    """
    更新榜单缓存
    """
    # 1. 从数据库计算最新排名
    ranking_data = calculate_ranking(dimension, group_type, season_id)

    # 2. 写入Redis
    cache_key = CACHE_KEYS['ranking'].format(
        dimension=dimension,
        group_type=group_type,
        season_id=season_id
    )
    redis_client.setex(
        cache_key,
        300,  # TTL: 5分钟
        json.dumps(ranking_data)
    )

    # 3. 发布缓存更新通知(用于WebSocket推送)
    redis_client.publish(f'ranking_updates:{dimension}:{group_type}', json.dumps({
        'action': 'updated',
        'data': ranking_data
    }))
```

#### 缓存失效策略

```python
# 主动失效
def invalidate_game_cache(game_id: int):
    """
    失效游戏相关缓存
    """
    # 失效游戏得分缓存
    redis_client.delete(CACHE_KEYS['game_score'].format(game_id=game_id))

    # 失效该游戏涉及的6个维度榜单
    for dimension in DIMENSIONS:
        for group_type in ['participant', 'judge']:
            redis_client.delete(
                CACHE_KEYS['ranking'].format(
                    dimension=dimension,
                    group_type=group_type,
                    season_id=CURRENT_SEASON
                )
            )

# TTL自动失效
# - 游戏得分缓存: 5分钟TTL
# - 榜单缓存: 5分钟TTL
# - 全局均值缓存: 1小时TTL(变化较少)
```

### 3.3 重算机制

#### 触发条件

```python
RECALCULATION_TRIGGERS = {
    'realtime': [
        'on_rating_submitted',          # 新评分提交
        'on_rating_modified',           # 评分修改
        'on_cheating_confirmed',        # 确认作弊
        'on_cheating_cleared',          # 误报恢复
    ],
    'scheduled': [
        'daily_2am',                    # 每天凌晨2点
        'after_cheating_review_batch',  # 作弊复核批处理
    ],
    'manual': [
        'admin_triggered',              # 管理员手动触发
        'algorithm_param_changed',      # 算法参数调整
    ],
}
```

#### 重算任务设计

```python
@app.task(bind=True, max_retries=3)
def async_recalculate_game(self, game_id: int):
    """
    异步重算单个游戏
    """
    try:
        # 1. 获取游戏所有维度的最新得分
        scores = calculate_all_dimension_scores(game_id)

        # 2. 更新数据库
        update_game_dimension_scores(game_id, scores)

        # 3. 更新缓存
        update_game_cache(game_id, scores)

        # 4. 触发相关榜单重算
        for dimension in scores.keys():
            trigger_ranking_recalculation(dimension)

    except Exception as exc:
        # 重试机制(最多3次)
        raise self.retry(exc=exc, countdown=60)  # 1分钟后重试
```

---

## 四、监控与告警

### 4.1 关键监控指标

#### 业务指标

```yaml
# Prometheus监控指标
business_metrics:
  - name: scoring_total_count
    type: counter
    description: 总评分数
    labels: [dimension, rater_type]

  - name: scoring_frequency_per_user
    type: gauge
    description: 用户评分频率(10分钟窗口)
    labels: [user_id]

  - name: ranking_calculation_duration
    type: histogram
    description: 排名计算耗时
    labels: [dimension, group_type]
    buckets: [0.1, 0.5, 1, 2, 5, 10, 30]

  - name: suspicious_rating_count
    type: gauge
    description: 可疑评分数量
    labels: [detection_type]

  - name: ranked_games_count
    type: gauge
    description: 进入排名的游戏数
    labels: [dimension, group_type]
```

#### 技术指标

```yaml
technical_metrics:
  - name: api_request_duration
    type: histogram
    description: API请求耗时
    labels: [endpoint, method]
    buckets: [0.01, 0.05, 0.1, 0.5, 1, 2, 5]

  - name: cache_hit_ratio
    type: gauge
    description: 缓存命中率
    labels: [cache_type]

  - name: database_query_duration
    type: histogram
    description: 数据库查询耗时
    labels: [table, operation]
    buckets: [0.001, 0.005, 0.01, 0.05, 0.1, 0.5]

  - name: message_queue_size
    type: gauge
    description: 消息队列积压量
    labels: [queue_name]
```

#### 数据一致性指标

```yaml
consistency_metrics:
  - name: data_consistency_check_status
    type: gauge
    description: 数据一致性检查结果(0=正常, 1=异常)
    labels: [check_type]

  - name: ranking_vs_db_games_mismatch
    type: gauge
    description: 榜单游戏数与DB不一致数量
    labels: [dimension, group_type]
```

### 4.2 告警规则

```yaml
# Prometheus告警规则
groups:
  - name: scoring_system_alerts
    interval: 30s
    rules:
      # 告警1: 可疑评分数量异常
      - alert: HighSuspiciousRatingCount
        expr: suspicious_rating_count > 50
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: "可疑评分数量过多"
          description: "过去5分钟内发现{{ $value }}个可疑评分"

      # 告警2: 排名计算耗时过长
      - alert: RankingCalculationSlow
        expr: histogram_quantile(0.95, ranking_calculation_duration) > 30
        for: 10m
        labels:
          severity: critical
        annotations:
          summary: "排名计算耗时过长"
          description: "P95耗时超过30秒: {{ $value }}秒"

      # 告警3: 数据一致性异常
      - alert: DataConsistencyCheckFailed
        expr: data_consistency_check_status > 0
        for: 1m
        labels:
          severity: critical
        annotations:
          summary: "数据一致性检查失败"
          description: "检测到数据不一致,请立即检查"

      # 告警4: 缓存命中率过低
      - alert: LowCacheHitRatio
        expr: cache_hit_ratio < 0.5
        for: 15m
        labels:
          severity: warning
        annotations:
          summary: "缓存命中率过低"
          description: "缓存命中率仅为{{ $value }},建议检查"

      # 告警5: 消息队列积压
      - alert: MessageQueueBacklog
        expr: message_queue_size > 1000
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: "消息队列积压"
          description: "队列{{ $labels.queue_name }}积压{{ $value }}条消息"
```

### 4.3 监控大盘

#### Grafana面板设计

```
┌──────────────────────────────────────────────────────────────┐
│                    评分系统监控大盘                           │
├──────────────────────────────────────────────────────────────┤
│                                                              │
│  [核心指标]                                                   │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐       │
│  │ 今日总评分   │  │ 可疑评分数   │  │ 排名游戏数   │       │
│  │    1,234     │  │     12       │  │    156       │       │
│  └──────────────┘  └──────────────┘  └──────────────┘       │
│                                                              │
│  [性能指标]                                                   │
│  ┌─────────────────────────────────────────────────────┐     │
│  │ 排名计算耗时分布 (P50/P95/P99)                      │     │
│  │ P50: 0.8s  P95: 2.3s  P99: 5.1s                    │     │
│  └─────────────────────────────────────────────────────┘     │
│                                                              │
│  [缓存性能]                                                   │
│  ┌─────────────────────────────────────────────────────┐     │
│  │ 缓存命中率趋势 (最近24小时)                          │     │
│  │ ████████████████████████████████████████  85%       │     │
│  └─────────────────────────────────────────────────────┘     │
│                                                              │
│  [数据一致性]                                                 │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐       │
│  │ 评分总数校验 │  │ 榜单校验     │  │ 全局均值校验 │       │
│  │   ✅ 通过    │  │   ✅ 通过    │  │   ✅ 通过    │       │
│  └──────────────┘  └──────────────┘  └──────────────┘       │
│                                                              │
└──────────────────────────────────────────────────────────────┘
```

---

## 五、回滚与应急预案

### 5.1 特性开关(Feature Flag)

```python
# 特性开关配置
FEATURE_FLAGS = {
    'scoring_algorithm_v1': {
        'enabled': True,
        'description': '第一届评分算法(截尾均值+贝叶斯平滑)',
        'config': {
            'trim_percentage': 0.1,       # 截尾比例10%
            'bayesian_k': 20,             # 贝叶斯k值
            'ranking_threshold': 15,      # 排名门槛
            'judge_weight': 2,            # 评审团权重
        }
    },

    'scoring_algorithm_v2': {
        'enabled': False,
        'description': '第二届评分算法(待定)',
        'config': {
            # 未来可以启用新算法
        }
    },

    'anti_cheat_strict_mode': {
        'enabled': True,
        'description': '严格反作弊模式',
    },

    'realtime_recalculation': {
        'enabled': True,
        'description': '实时重算(评分阶段)',
    },
}

# 使用特性开关
def get_algorithm_config():
    """
    获取当前算法配置
    """
    if FEATURE_FLAGS['scoring_algorithm_v1']['enabled']:
        return FEATURE_FLAGS['scoring_algorithm_v1']['config']
    elif FEATURE_FLAGS['scoring_algorithm_v2']['enabled']:
        return FEATURE_FLAGS['scoring_algorithm_v2']['config']
    else:
        raise Exception("未启用任何评分算法")
```

### 5.2 算法参数热更新

```python
# 配置管理
class AlgorithmConfig:
    """
    算法配置管理器
    """
    @staticmethod
    def get_config(season_id: str) -> dict:
        """
        从数据库获取配置
        """
        config = AlgorithmConfigModel.objects.get(season_id=season_id)
        return {
            'trim_percentage': config.trim_percentage,
            'bayesian_k': config.bayesian_k,
            'ranking_threshold': config.ranking_threshold,
            'judge_weight': config.judge_weight,
        }

    @staticmethod
    def update_config(season_id: str, new_config: dict, admin_user_id: int):
        """
        更新配置(需要管理员权限)
        """
        with transaction.atomic():
            # 1. 保存旧配置
            old_config = AlgorithmConfig.objects.get(season_id=season_id)

            # 2. 更新配置
            AlgorithmConfig.objects.filter(season_id=season_id).update(**new_config)

            # 3. 记录审计日志
            AuditLog.objects.create(
                admin_user_id=admin_user_id,
                action='update_algorithm_config',
                old_value=json.dumps(old_config.to_dict()),
                new_value=json.dumps(new_config),
            )

            # 4. 触发全量重算
            trigger_full_recalculation(season_id)

            # 5. 清除所有缓存
            invalidate_all_caches(season_id)
```

### 5.3 数据修复SOP

#### 场景1: 赛后发现作弊账号

```python
def handle_cheating_account_after_season(cheater_user_id: int, season_id: str):
    """
    赛后处理作弊账号
    """
    # 步骤1: 标记作弊账号
    User.objects.filter(id=cheater_user_id).update(
        is_banned=True,
        ban_reason='作弊',
        banned_at=timezone.now()
    )

    # 步骤2: 作废其评分
    Rating.objects.filter(
        rater_id=cheater_user_id,
        season_id=season_id
    ).update(
        is_valid=False,
        invalid_reason='作弊账号作废'
    )

    # 步骤3: 重算排名
    recalculate_all_rankings(season_id)

    # 步骤4: 通知受影响的创作者
    affected_creators = get_affected_creators(cheater_user_id)
    for creator in affected_creators:
        send_notification(
            user_id=creator.id,
            title='排名已调整',
            message=f'因作弊账号处理,您的游戏排名已调整',
        )

    # 步骤5: 公告更新
    announce_ranking_update(
        reason='处理作弊账号',
        affected_games=len(affected_creators)
    )
```

#### 场景2: 算法Bug修复

```python
def handle_algorithm_bug(fixed_version: str, season_id: str):
    """
    处理算法Bug修复
    """
    # 步骤1: 备份当前榜单
    backup_current_rankings(season_id, backup_reason=f'bug_fix_{fixed_version}')

    # 步骤2: 更新算法代码(部署新版本)
    # (通过CI/CD流程)

    # 步骤3: 全量重算
    recalculate_all_rankings(season_id)

    # 步骤4: 对比新旧榜单
    old_rankings = load_backup_rankings(season_id)
    new_rankings = get_current_rankings(season_id)

    changes = compare_rankings(old_rankings, new_rankings)

    # 步骤5: 如果变化较大,需要公告
    if changes.significant:
        announce_ranking_update(
            reason='算法修复',
            details=changes.summary
        )
```

#### 场景3: 数据异常回滚

```python
def rollback_to_backup(season_id: str, backup_version: int):
    """
    回滚到备份版本
    """
    # 步骤1: 确认回滚(需要二次确认)
    confirm = input(f"确认回滚到备份版本{backup_version}? (yes/no): ")
    if confirm != 'yes':
        return

    # 步骤2: 备份当前数据(以防万一)
    backup_current_rankings(season_id, backup_reason='pre_rollback')

    # 步骤3: 恢复备份数据
    with transaction.atomic():
        # 3.1 恢复game_dimension_scores表
        restore_table_from_backup(
            table_name='game_dimension_scores',
            backup_version=backup_version
        )

        # 3.2 恢复rankings表
        restore_table_from_backup(
            table_name='rankings',
            backup_version=backup_version
        )

        # 3.3 清除所有缓存
        invalidate_all_caches(season_id)

    # 步骤4: 记录回滚日志
    AuditLog.objects.create(
        action='rollback_rankings',
        details=f'回滚到备份版本{backup_version}',
    )

    # 步骤5: 通知用户
    send_system_notification(
        title='榜单已回滚',
        message='因数据异常,榜单已回滚到上一版本',
    )
```

### 5.4 应急预案

#### 预案1: 评分提交服务异常

```yaml
incident_type: 评分提交服务异常
severity: P0

detection:
  - api_error_rate > 5%
  - api_latency_p99 > 10s

response_steps:
  1. 立即检查服务日志
  2. 如果是数据库问题:
     - 切换到只读模式(暂停评分提交)
     - 显示"评分服务暂时不可用"提示
  3. 如果是代码Bug:
     - 回滚到上一版本
  4. 恢复后:
     - 补充丢失的评分数据(如果有)
     - 触发全量重算

recovery_time_objective: 15分钟内恢复
```

#### 预案2: 排名计算服务异常

```yaml
incident_type: 排名计算服务异常
severity: P1

detection:
  - ranking_calculation_duration_p99 > 60s
  - message_queue_size > 5000

response_steps:
  1. 立即检查计算服务日志
  2. 如果是性能问题:
     - 扩容计算节点
     - 降低实时重算频率(从5分钟改为15分钟)
  3. 如果是数据问题:
     - 暂停实时重算
     - 手动修复异常数据
     - 恢复后全量重算
  4. 显示"排名数据更新延迟"提示

recovery_time_objective: 30分钟内恢复
```

#### 预案3: 数据一致性异常

```yaml
incident_type: 数据一致性异常
severity: P0

detection:
  - data_consistency_check_status > 0
  - 评分总数 vs 人次总数不一致
  - 榜单游戏数 vs DB游戏数不一致

response_steps:
  1. 立即暂停排名展示(显示"数据维护中")
  2. 定位不一致原因
  3. 如果是计算逻辑问题:
     - 修复Bug
     - 全量重算
  4. 如果是数据损坏:
     - 从备份恢复
     - 全量重算
  5. 恢复后进行数据一致性验证
  6. 公告用户"排名数据已修正"

recovery_time_objective: 1小时内恢复
```

---

## 六、API接口规范

### 6.1 评分相关API

#### 提交评分

```http
POST /api/v1/ratings HTTP/1.1
Content-Type: application/json
Authorization: Bearer <token>

{
  "game_id": 123,
  "innovation_score": 8.5,
  "theme_score": 7.0,
  "visual_score": 9.0,
  "audio_score": 8.0,
  "completeness_score": 7.5,
  "fun_factor_score": 8.5
}

# 响应
HTTP/1.1 201 Created
{
  "id": 456,
  "game_id": 123,
  "status": "submitted",
  "message": "评分提交成功,排名将在5分钟内更新"
}
```

#### 修改评分

```http
PUT /api/v1/ratings/{rating_id} HTTP/1.1
Content-Type: application/json
Authorization: Bearer <token>

{
  "innovation_score": 9.0,
  "theme_score": 7.5
}

# 响应
HTTP/1.1 200 OK
{
  "id": 456,
  "status": "updated",
  "message": "评分修改成功,排名将在5分钟内更新"
}
```

### 6.2 排名相关API

#### 获取榜单

```http
GET /api/v1/rankings?dimension=innovation&group=participant&season=season1 HTTP/1.1

# 响应
HTTP/1.1 200 OK
{
  "dimension": "innovation",
  "group_type": "participant",
  "season_id": "season1",
  "total_games": 156,
  "rankings": [
    {
      "rank": 1,
      "game_id": 123,
      "game_title": "星际探索者",
      "score": 8.425,
      "rating_count": 50,
      "variance": 0.8
    },
    {
      "rank": 2,
      "game_id": 456,
      "game_title": "像素冒险",
      "score": 8.425,
      "rating_count": 48,
      "variance": 0.9
    }
  ],
  "last_updated": "2025-01-10T12:00:00Z"
}
```

### 6.3 管理员API

#### 手动触发重算

```http
POST /api/v1/admin/rankings/recalculate HTTP/1.1
Content-Type: application/json
Authorization: Bearer <admin_token>

{
  "season_id": "season1",
  "force": true,
  "reason": "算法参数调整后重算"
}

# 响应
HTTP/1.1 202 Accepted
{
  "task_id": "recalc-task-123",
  "status": "queued",
  "estimated_time": "5-10分钟",
  "message": "重算任务已加入队列"
}
```

#### 查看重算状态

```http
GET /api/v1/admin/rankings/recalculate/status/{task_id} HTTP/1.1

# 响应
HTTP/1.1 200 OK
{
  "task_id": "recalc-task-123",
  "status": "processing",
  "progress": {
    "current": 5,
    "total": 12,
    "percentage": 42
  },
  "started_at": "2025-01-10T12:00:00Z",
  "estimated_completion": "2025-01-10T12:08:00Z"
}
```

### 6.4 错误码规范

```json
{
  "error_codes": {
    "SCORING_PHASE_ENDED": {
      "code": 4001,
      "message": "评分阶段已结束",
      "http_status": 403
    },
    "ALREADY_RATED": {
      "code": 4002,
      "message": "您已经评价过该游戏",
      "http_status": 409
    },
    "RATE_LIMIT_EXCEEDED": {
      "code": 4003,
      "message": "评分频率过高,请稍后再试",
      "http_status": 429
    },
    "GAME_NOT_FOUND": {
      "code": 4004,
      "message": "游戏不存在",
      "http_status": 404
    },
    "RANKING_NOT_PUBLISHED": {
      "code": 4005,
      "message": "排名尚未公布",
      "http_status": 404
    }
  }
}
```

---

## 七、部署与运维

### 7.1 部署架构

```
┌─────────────────────────────────────────────────────────┐
│                      负载均衡器                           │
│                   (Nginx / ALB)                          │
└──────────────┬──────────────────────────────────────────┘
               │
┌──────────────▼──────────────────────────────────────────┐
│                    应用服务器集群                         │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐     │
│  │  App Server │  │  App Server │  │  App Server │     │
│  │     #1      │  │     #2      │  │     #3      │     │
│  └─────────────┘  └─────────────┘  └─────────────┘     │
└─────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────┐
│                   后台任务集群                            │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐     │
│  │  Celery     │  │  Celery     │  │  Celery     │     │
│  │  Worker #1  │  │  Worker #2  │  │  Worker #3  │     │
│  └─────────────┘  └─────────────┘  └─────────────┘     │
└─────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────┐
│                    数据层                                │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐     │
│  │ PostgreSQL  │  │    Redis    │  │  RabbitMQ   │     │
│  │  (主从复制)  │  │  (主从复制)  │  │   (集群)    │     │
│  └─────────────┘  └─────────────┘  └─────────────┘     │
└─────────────────────────────────────────────────────────┘
```

### 7.2 部署流程

#### 标准部署

```bash
# 1. 拉取最新代码
git pull origin main

# 2. 安装依赖
pip install -r requirements.txt

# 3. 数据库迁移
python manage.py migrate

# 4. 构建前端资源
npm run build

# 5. 重启应用服务
systemctl restart jingzhe-ratings-api
systemctl restart jingzhe-ratings-worker

# 6. 健康检查
curl http://localhost:8000/health
```

#### 滚动部署(零停机)

```bash
# 1. 部署到第一个节点
deploy_to_server server1
wait_for_health_check server1

# 2. 部署到第二个节点
deploy_to_server server2
wait_for_health_check server2

# 3. 部署到第三个节点
deploy_to_server server3
wait_for_health_check server3

# 4. 清理旧版本
cleanup_old_versions
```

### 7.3 容量规划

#### 第一届规模预估

```
预期规模:
- 参赛游戏数: 50-100个
- 参赛者数: 80-150人
- 评审团数: 5-10人
- 总评分数: 1000-3000条

数据量估算:
- 评分数据: 3000条 × 6维度 × 100字节 ≈ 1.8MB
- 排名数据: 12榜单 × 100游戏 × 200字节 ≈ 240KB
- 缓存数据: 5分钟TTL,约10MB

数据库规格:
- PostgreSQL: 2核4GB,存储50GB
- Redis: 1核2GB,存储10GB
- 应用服务器: 2核4GB × 3台
```

#### 性能基准

```
目标性能:
- 评分提交API: P95 < 200ms
- 榜单查询API: P95 < 100ms
- 排名计算: P95 < 10s(全量重算)
- 缓存命中率: > 80%

并发能力:
- 评分提交: 100 QPS
- 榜单查询: 500 QPS
- 排名重算: 支持每日10次全量重算
```

---

## 八、附录

### 8.1 技术栈选型

| 组件 | 技术选型 | 理由 |
|------|---------|------|
| 后端框架 | Django + Django REST Framework | 成熟稳定,生态完善 |
| 数据库 | PostgreSQL 14+ | 支持JSONB,性能优秀 |
| 缓存 | Redis 6+ | 高性能,支持丰富数据结构 |
| 消息队列 | RabbitMQ / Celery | 可靠的任务队列 |
| 监控 | Prometheus + Grafana | 开源监控方案 |
| 日志 | ELK Stack (Elasticsearch + Logstash + Kibana) | 日志聚合与分析 |
| 部署 | Docker + Kubernetes | 容器化部署 |

### 8.2 相关文档索引

- [评分系统设计方案 v2.5](../design/评分系统设计方案_v1.0.md)
- [排名系统技术实现文档 v1.0](../design/archive/ranking_system_v1_deprecated/排名系统技术实现文档_v1.0.md)
- [评分系统实现缺口清单](../development/active/issues/scoring-system-implementation-gaps_2025-01-10.md)
- [关键名词解释索引 v1.8](../design/关键名词解释索引_v1.0.md)

### 8.3 版本历史

| 日期 | 版本 | 变更内容 | 作者 |
|------|------|---------|------|
| 2025-01-10 | v1.0 | 初始版本,完成系统架构设计 | Claude |

---

**文档维护者**: 老黑(Claude)
**状态**: 🔄 初稿
**下一步**: 补充API设计文档、数据库设计文档、运维手册
